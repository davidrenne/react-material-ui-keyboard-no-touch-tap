...
import Dialog from "velocity-mui/Dialog";
...
import getMuiTheme from "velocity-mui/styles/getMuiTheme";
...
ActiveElement.isInput = () =>
  document.activeElement.tagName.toLowerCase() === constants.input;
...
export class Keyboard extends React.Component<KeyboardProps, KeyboardState> {
  private static calculatedTextFieldHeight(
    props: TextFieldAccessedProps
  ): number {
    const { rows, floatingLabelText } = props;
    const normalHeight: number = floatingLabelText
      ? constants.seventyTwo
      : constants.fourtyEight;
    return (
      (rows ? (rows - constants.one) * constants.twentyFour : constants.zero) +
      normalHeight
    );
  }
  ...
  private static supportedSpecialKeys: Array<string> = [
    "Enter",
    "Backspace",
    "Escape",
    "CapsLock",
  ...
  private static overwrittenProps: Array<string> = [
    "onChange",
    "onFocus",
    "onBlur",
    "onKeyUp",
  ...
  public static automaitcOpenPredicate: AutomaitcOpenPredicate =
    allwaysTruePredicate;
    ...
    switch (key) {
      ...
      case supportedSpecialKeys[constants.one]:
        return this.setValue(
          value.substring(constants.zero, value.length - constants.one)
        );
      case supportedSpecialKeys[constants.two]:
        return this.requestClose();
      case supportedSpecialKeys[constants.three]:
        return this.setState({ capsLock: !capsLock });
      case supportedSpecialKeys[constants.four]:
        return this.setState({
          layout:
            layout === propsLayout.length - constants.one
              ? constants.zero
              : layout! + constants.one,
        });
      default:
        return this.setValue(value + key);
    }
  }
  ...
  private onKeyDown(event: KeyboardEvent): void {
    ...
    if (
      key.length === constants.one ||
      Keyboard.getSupportedSpecialKeys().indexOf(key) !== constants.minusOne
    ) {
  ...
  public componentDidMount(): void {
    EventListenerService.addListener(
      constants.resize,
      this.onResize,
      constants.boolFalse
    );
  ...
  public shouldComponentUpdate(
    ...
    if (
      !deepEqual(this.props.layouts, props.layouts, constants.strictCompare)
    ) {
      return constants.boolTrue;
    }
    if (
      !deepEqual(thisTextField.props, textField.props, constants.strictCompare)
    ) {
      return constants.boolTrue;
    }
  ...
  public componentDidUpdate(props: KeyboardProps, state: KeyboardState): void {
    ...
    if (open !== prev) {
      if (open) {
        ...
        EventListenerService.removeListener(
          constants.keydown,
          this.onKeyDown,
          constants.boolTrue
        );
  ...
  public render(): JSX.Element {
    ...
    let inputTextFieldProps: TextFieldAccessedProps = objectAssign(
      {},
      textField.props,
      { readOnly: open }
    );
    ...
    let prop: string;
    for (
      propIndex = constants.zero;
      propIndex < overwrittenPropsLength;
      ++propIndex
    ) {
    ...
    const inputTextField: TextFieldElement = React.cloneElement(
      textField,
      inputTextFieldProps
    );
    const keyboardTextField: TextFieldElement = React.createElement(
      textField.type as CreatableTextField,
      keyboardFieldProps
    );
    const keyboardLayout: KeyboardLayout = kyeboardCapsLockLayout(
      layouts[stateLayout!],
      capsLock!
    );
    ...
    let keyHeight: number =
      typeof keyboardKeyHeight === constants.typeofNumber
        ? keyboardKeyHeight!
        : theme.button!.height!;
    let keyWidth: number =
      typeof keyboardKeyWidth === constants.typeofNumber
        ? keyboardKeyWidth!
        : theme.button!.minWidth!;
    let keySymbolSize: number =
      typeof keyboardKeySymbolSize === constants.typeofNumber
        ? keyboardKeySymbolSize!
        : theme.flatButton!.fontSize!;
    const { desktopGutter, desktopKeylineIncrement } =
      theme.baseTheme!.spacing!;
    ...
    const styleHeight: number = minHeight
      ? minHeight
      : height
      ? height
      : maxHeight
      ? maxHeight
      : constants.zero;
    const textFieldHeight: number =
      styleHeight > constants.zero
        ? styleHeight
        : Keyboard.calculatedTextFieldHeight(inputTextFieldProps);
    ...
    let dialogHeight: number =
      keyboardRowLength * keyHeight + textFieldHeight + dialogGutter;
    ...
    if (overwriteWidth || overwriteHeight) {
      ...
      if (overwriteHeight) {
        if (dialogSpacingTop >= constants.zero) {
          ...
          keyHeight =
            (dialogHeight - textFieldHeight - dialogGutter) / keyboardRowLength;
        }
      }
      const smallerSymbolSize: number =
        (keyHeight < keyWidth ? keyHeight : keyWidth) - constants.four;
    ...
    for (
      let rowIndex: number = constants.zero;
      rowIndex < keyboardRowLength;
      ++rowIndex
    ) {
      keyboardRowKeys = [];
      row = keyboardLayout[rowIndex];
      ...
      for (keyIndex = constants.zero; keyIndex < rowLength; ++keyIndex) {
        ...
        keyboardRowKeys.push(
          <KeyboardKey
            ...
            keyboardKeyWidth={
              keyWidth * (notSpacebar ? constants.one : key.length)
            }
            keyboardKeySymbolSize={keySymbolSize}
            disableEffects={
              disableEffects ? constants.boolTrue : constants.boolFalse
            }
          />
        );
      }
    ...
    return muiTheme ? (
      ...
      <MuiThemeProvider>{keyboard}</MuiThemeProvider>
    );
  }
}
